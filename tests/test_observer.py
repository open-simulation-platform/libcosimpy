...
def test_from_last_value_observer_read(test_dir):
    if platform() == "Windows":
        ...
        integer_value_list = observer.last_integer_values(slave_index=slave_index,
                                                          variable_references=integer_variables_list)
        assert integer_value_list[0] == 1000
        boolean_variables_list = [0]
        boolean_value_list = observer.last_boolean_values(slave_index=slave_index,
                                                          variable_references=boolean_variables_list)
        assert boolean_value_list[0]
        string_variables_list = [0]
        string_value_list = observer.last_string_values(slave_index=slave_index,
                                                        variable_references=string_variables_list)
...
def test_from_time_series(test_dir):
    ...
    real_time_points, real_step_numbers, real_samples = observer.time_series_real_samples(0,
    ...
    integer_time_points, integer_step_numbers, integer_samples = observer.time_series_integer_samples(0,
                                                                                                      value_reference=value_reference,
                                                                                                      sample_count=10,
                                                                                                      from_step=from_step)
    ...
    assert observer.stop_time_series(slave_index=0, value_reference=value_reference,
    ...
    assert observer1.start_time_series(slave_index=0, value_reference=value_reference,
                                       variable_type=CosimVariableType.INTEGER)
    execution.step(step_count=2)
    from_step1 = 7
    integer_time_points1, integer_step_numbers1, integer_samples1 = observer1.time_series_integer_samples(slave_index=0,
                                                                                                          value_reference=value_reference,
                                                                                                          sample_count=10,
    ...
    assert observer2.start_time_series(slave_index=0, value_reference=value_reference,
                                       variable_type=CosimVariableType.INTEGER)
    execution.step(step_count=5)
    from_step2 = 7
    integer_time_points2, integer_step_numbers2, integer_samples2 = observer2.time_series_integer_samples(slave_index=0,
                                                                                                          value_reference=value_reference,
                                                                                                          sample_count=10,
