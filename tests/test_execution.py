...
def test_initial_value(test_dir):
    ...
    real_value = 1.2
    assert execution.real_initial_value(slave_index=slave_index, variable_reference=variable_reference,
                                        value=real_value)
    integer_value = -5
    assert execution.integer_initial_value(slave_index=slave_index, variable_reference=variable_reference,
                                           value=integer_value)
    boolean_value = True
    assert execution.boolean_initial_value(slave_index=slave_index, variable_reference=variable_reference,
                                           value=boolean_value)
    string_value = "Hello World!"
    assert execution.string_initial_value(slave_index=slave_index, variable_reference=variable_reference,
                                          value=string_value)
    execution.step()
